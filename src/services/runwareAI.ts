...
export class RunwareService {
  ...
  async generateQuoteImage(quoteData: any): Promise<string> {
    const prompt = `Professional sales quote design with elegant typography and modern layout. Company proposal for ${quoteData.company || 'business'} including pricing table, clean white background, corporate blue accents, professional font. Total value $${quoteData.total?.toLocaleString() || '0'} prominently displayed. High-quality business document design.`;
...
export let runwareService: RunwareService | null = null;

export function initializeRunware(apiKey: string): RunwareService {
  runwareService = new RunwareService(apiKey);
  return runwareService;
}

export function getRunwareService(): RunwareService | null {
  return runwareService;
}